...
1. Use the figma-design-extractor subagent to provide a design guide for the feature, citing the `figma hint`, and tell it to extract only design information which will be useful for implementation, such as:
...
2. Summarise the feedback from the subagent and leave a reference to the figma file/component for future lookups if needed. 
3. If subagent feedback fails, record a note like:  
...
Create a markdown spec document that Plan mode can use directly and save it in the _specs folder using the `feature_slug`. Use the exact structure as defined in the spec template file here: @_specs/template.md. Include only the main test cases that should be tested, as a starting point.
